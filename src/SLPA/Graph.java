...
//			}
			...
			while ((line = br.readLine()) != null) {
				...
				int value = Integer.parseInt(config[2]);
		...
		for (Map.Entry<Integer, Node> entry : graphADT.entrySet()) {
			...
			Map<Node,Integer> neigh = value.getNeighbhours();
			no += neigh.size();
		}
		if(no/2 != edges) {
	 ...
	 */
	//source->dest
	private void addEdge(Integer source, Integer dest,Integer value) {
		...
		destNode.addNeighbour(sourceNode,value);
	}
